...
最小コマンドはどこまで素朴に書けたのか、縦長・横長・1:1 は通ったのか、
1 枚から複数枚に増えるとどこで摩擦があったのか、`image_generation` は
最初どう見えたのか、といった実地の観察を、動かした順に残しています。
...
5. [`image_generation` は既定で無効だったので、私が試した 2 通り](#image_generation-は既定で無効だったので私が試した-2-通り)
6. [最小コマンドでの動作確認結果](#最小コマンドでの動作確認結果)
7. [`printf` を使った理由と読み方](#printf-を使った理由と読み方)
8. [日本語 prompt と英語 prompt](#日本語-prompt-と英語-prompt)
9. [アスペクト比で見えた現実](#アスペクト比で見えた現実)
10. [1 枚から複数枚へ — 試しに作ってみた一括実行スクリプトの位置づけ](#1-枚から複数枚へ--試しに作ってみた一括実行スクリプトの位置づけ)
11. [今回辿った doctor → preview → run の順](#今回辿った-doctor--preview--run-の順)
12. [JSON spec の形](#json-spec-の形)
13. [組み込みプリセット](#組み込みプリセット)
14. [よく流れてくる話と、今回の観察の対比](#よく流れてくる話と今回の観察の対比)
15. [外部に共有する前の自分向けチェック](#外部に共有する前の自分向けチェック)
16. [オプション早見表](#オプション早見表)
17. [作業中に観察した誤りやすい点](#作業中に観察した誤りやすい点)
18. [参照した公式ドキュメント](#参照した公式ドキュメント)

---

## はじめに（最低限の前提だけ）

この文書は、Codex CLI と WSL2 に最低限の知識がある読者が、私の観察を
追試できるよう整えたメモです。初心者向けの導入解説ではないので、個々の
ツールの入門情報は各公式ドキュメントを優先してください。
...
以降の記述は、この前提の上で私が実際に実行したコマンドと、その結果の
...
いずれかに寄る様子でした。同梱スクリプトの aspect preset も、その観察を前提に
...
私が作業中に踏んだ、あるいは周囲でよく聞いた誤解を、観察として並べます。
同じ条件で作業される方には、私の失敗の履歴として参考になるかもしれま
せん。注意書きではなく、観察の共有です。
