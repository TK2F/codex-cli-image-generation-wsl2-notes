...
この章は、Codex CLI も WSL2 もまだ触ったことがない読者向けの軽い補足です。
すでに慣れている方はそのまま次の節へどうぞ。

- **Codex CLI**: OpenAI が提供するコマンドラインツールで、`codex` という
  コマンド名で起動します。対話モードと、`codex exec` のような非対話実行
  モードの両方を持ちます。公式: https://developers.openai.com/codex/cli
- **WSL2**: Windows 10 / 11 の上で Linux をそのまま動かせる Microsoft
  公式の仕組みです。スタートメニューから「Ubuntu」を開くと Linux 側の
  ターミナル（= Bash）が立ち上がります。未導入であれば、初回だけ管理者
  PowerShell で `wsl --install` を実行します。公式:
  https://learn.microsoft.com/windows/wsl/install
- **PowerShell と Bash は別物**: 見た目は似ていますが、コマンドの
  エスケープやパイプの扱いが異なります。この文書のコマンドはすべて
  **WSL 側の Bash** を想定しています。Windows PowerShell に貼り付けても
  多くの場合そのままでは動きません。
- **コードブロックの読み方**: 本文中の先頭に `$` やプロンプト記号は
  書いていません。ブロックの内容をそのままターミナルに貼り付けて
  Enter で実行できます。`# ...` はコメントなので無視されます。
- **前提状態**: この文書は、WSL2 Ubuntu の中に `codex` が導入済みで、
  初回のログインとサンドボックス設定が済んでいる状態を起点にしています。
  未導入の場合は上記の Codex CLI 公式ドキュメントから進めてください。
- **副作用の大きさ**: 本文のコマンドは、読むだけなら副作用が小さいものか
  ら並んでいます。`--doctor` と `--preview` は画像を生成しません。

以降は、この前提の上で実際に試した順の記録です。
...
最初に気になるのは、たぶん「本当に 1 行で通るのか」の 1 点だけでしょう。
この環境で実際に通った最小コマンドを、生成 → 編集（1 枚）→ 編集（2 枚）の
順に並べます。
...
## 1 枚から複数枚へ — runner が欲しくなったとき

1 枚だけなら one-liner で足ります。ただし、何枚かをまとめて回す段階で、
次の 7 点を毎回自分で書くのが面倒になってきました。

- 本実行前に prompt と command を目視確認したい（preview）
- 失敗したジョブを自動で retry したい
- ジョブ間の間隔を空けたい
- Linux パス / Windows ドライブパス / WSL UNC パスが混ざっても扱いたい
- Codex が PNG を指定先にコピーしなかったときに
  `~/.codex/generated_images` から回収したい
- 1 回の run で何が成功し何が失敗したかを summary JSON に残したい
- ジョブ単位の raw log を残したい

`codex-image-batch.sh` は、この 7 点だけを素朴にカバーするための Bash
実装です。必須の外部依存は `jq` と `python3` のみで、読み切れる長さに
おさめてあります。

## 今回辿った doctor → preview → run の順

新しいマシンで最初に触ったとき、迷わなかった順序はこの 3 ステップでした。
読者の環境で同じ順でうまくいかない場合は、`--doctor` の出力を起点に
切り分けると原因に近づきやすいはずです。
...
自分が最初に踏んだ、または周囲で頻発している誤解を並べます。読む前に
知っていれば余計な時間を使わずに済む、という程度の注意書きです。
